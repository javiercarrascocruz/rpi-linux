 ...
 */
static int veml6031x00_set_power_state(struct veml6031x00_data *data,
					       bool on)
{
	u8 conf[2];
	int ret;
...
static const struct iio_chan_spec veml6031x00_channels[] = {
	{
		...
		.info_mask_shared_by_type_available = BIT(IIO_CHAN_INFO_INT_TIME),
...
static irqreturn_t veml6031x00_interrupt(int irq, void *private)
{
	...
	if ((regval & (VEML6031X00_INT_TH_H | VEML6031X00_INT_TH_L)) && data->ev_en) {
	...
	if ((regval & VEML6031X00_INT_DRDY) && data->trig_en)
